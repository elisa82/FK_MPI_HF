...
c
c      program fk
      Subroutine sub_bs_dc(nx,x,r_max,lnpt,dt,t0,gf8nx)
      include 'constants.h'
      include 'model.h'
      integer i,j,l,iblank,nfft,nfft2,n,ix,nx,tenPerc,count,total
      INTEGER ntmp,nCom, wc, tb, smth ,new_old(9)
      ...
      real qaw,qbw !JORGE frequency dependent Q
      complex w,at0,at,grt,u(3,6),cmu_freq
      real aj0,aj1,aj2,z,ddr,ddi,rwvs
      complex sum(ndis,9,nt/2), data(nt),tdata(9,nt)
      integer jo,nb,ncom_out,id_src
      real den(nlay),th(nlay),sig(9),gf8nx(nt*8*ndis)
...
c      read(*,*)mb,srclayer,src
      ...
      rwvs=rwvs/hs
      vs = b(srclayer)
c      write(0,'(a15,f8.1,a20,i3)') 'source depth =',hs,
c     +				   ' km at top of layer',srclayer
...
c      read(*,*) pmin,pmax,dk,kc
      ...
      dk=0.1
      xmax=amax1(r_max, hs)
c      xmax=sqrt(amax1((rtrav*rtrav-hs*hs), r_max*rmax))+r_max
      kc = kc/hs
      pmin = pmin/vs
c      pmax = pmax/vs
      pmax=pmax*rwvs
...
c      write(0,1001)'dk =',dk,'kmax =',kc,'pmax =',pmax,'N =',total
1001  format(a6,f9.5,a8,f6.2,a8,f6.4,a8,i9)
      ...
      do j=1,nfft2
        ...
        enddo
      enddo
c      write(0,'(a)') ' start F-K computation ...'
      do j=1, nfft2		! start frequency loop
         w = cmplx(omega,-sigma)	! complex frequency
         do i = 1, mb
            at = clog(w/pi2)/pi + cmplx(0.,0.5)		! A&R, p182
            if (omega >= 1) then
C            qbw=(34.0*b(i)+41.0)*(omega/pi2)**0.6
C            qbw=179.*(omega/pi2)**0.6           !Western US
C            qaw=2.*qbw                          !Western US
             qbw=718.*(omega/pi2)**0.45         !Easter US
             qaw=qbw                            !Eastern US
            else
C            qbw=(34.0*b(i)+41.0)*(1/pi2)**0.6   !Western US
C            qbw=179.*(1/pi2)**0.6               !Western US
C            qaw=2.*qbw                          !Western US
             qbw=718.*(1/pi2)**0.45             !Eastern US
             qaw=qbw                            !Eastern US
            endif
            ka(i) = w/(a(i)*(1.+at/qa(i)))
            kb(i) = w/(b(i)*(1.+at/qb(i)))
            ka(i) = ka(i)*ka(i)
            kb(i) = kb(i)*kb(i)
         enddo
         k= omega*pmin + 0.5*dk
         n=(sqrt(kc+(pmax*omega)**2)-k)/dk
c         n=(sqrt(kc+(pmax*omega)**2))/dk
...
c	      write(0,'(i4,a6)') int(100.*count/total)+1, '% done'
	    endif
	 enddo			! end of k-loop
	 lowpass = 1.
...
c      write(0,'(i9,a40)') count,' 100% done, writing files ... '
...
c     &			 %val(x(ix)),tdata)
	     ...
	     do j=1,nfft2
	       sum(ix,l,j) = tdata(l,j)*sig(l)
               ntmp=((ix-1)*ncom_out+l-1)*nfft+2*j
               gf8nx(ntmp-1)=real(sum(ix,l,j))
               gf8nx(ntmp  )=aimag(sum(ix,l,j))
	     enddo
           enddo
	 endif
      enddo
