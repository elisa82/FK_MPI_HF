...
c
       subroutine cmodel(nlayer,freq_ref,depth,cc,ss,dd,tth,qqa,qqb)
c
       implicit none
       include 'model.h'
       integer jo,nb,j,jj,j1,ncom_out,nlayer
       real freq_ref,depth,cc(nlay),ss(nlay),dd(nlay),
     &      tth(nlay),qqa(nlay),qqb(nlay)
       real c(nlay),s(nlay),den(nlay),th(nlay),qa(nlay),qb(nlay)
       real x,unrelaxed_mu,dep(nlay)
...
c
       ...
       nb=2
       do  j=1,jo
C         if(depth.gt.dep(j)) nb=j+1     !JORGE commented this to put
C         what's below
          if(depth.gt.dep(j).and.depth.lt.dep(j+1)) nb=j+1
       enddo
       th(nb-1)=depth-dep(nb-1)
       do j=nb,jo
         jj=jo+nb-j
         j1=jj+1
         th(j1)=th(jj)
...
C      jo=jo-1    !JORGE included this statmente because of line 49
      ns=nb
      do i=1,jo
        th(i)=thh(i)
        v(i)=1./c(i)**2
C        write(*,*)'th and v are:',jo,th(i),v(i)
      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      implicit none
      integer i, ns, lmax
      real*8 aminm, dtdp0, t0, x, pm, th(100), v(100)
